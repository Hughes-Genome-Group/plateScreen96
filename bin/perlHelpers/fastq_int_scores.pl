...
# Copied over, modified from NGseqBasic VS20, 21Feb2017
# Originally coded by Simon McGowan
...
sub get_scheme
{
		...
		if ($max_score <= 73) {return "33"}
		elsif ($max_score <= 74) {return "33"}
		elsif (($min_score >= 58) and ($min_score < 64)) {return "64"}
		elsif (($min_score == 64) or ($min_score == 65)) {return "64"}
		elsif ($min_score >= 66)  {return "64"}
		else {return "33"}
